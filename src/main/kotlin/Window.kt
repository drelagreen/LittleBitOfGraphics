    ...
    init {
        ...
        while (true) {
            ...
            if (time - lastRenderTime >= timeBetweenFrames) {
                /*Каждые N секунд вызываем метод repaint
                * Он у нас высокопроизводительно закрашивает весь канвас прямогуольником (затирает его)
                * После чего рендерит то что нам нужно
                *
                * в repaint можно указать область очистки канваса, если фигуры будут находиться за этой областью, то они не будут рендериться
                *                 canvas.repaint(0,0,100,100)
                *
                * */
